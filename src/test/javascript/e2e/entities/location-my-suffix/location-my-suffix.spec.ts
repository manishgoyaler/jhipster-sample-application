...
import { LocationComponentsPage, LocationDeleteDialog, LocationUpdatePage } from './location-my-suffix.page-object';
...
describe('Location e2e test', () => {
  ...
  it('should load Locations', async () => {
    await navBarPage.goToEntity('location-my-suffix');
    locationComponentsPage = new LocationComponentsPage();
    await browser.wait(ec.visibilityOf(locationComponentsPage.title), 5000);
    expect(await locationComponentsPage.getTitle()).to.eq('jhipsterSampleApplicationApp.location.home.title');
  ...
  it('should load create Location page', async () => {
    ...
    expect(await locationUpdatePage.getPageTitle()).to.eq('jhipsterSampleApplicationApp.location.home.createOrEditLabel');
  ...
  it('should delete last Location', async () => {
    ...
    expect(await locationDeleteDialog.getDialogTitle()).to.eq('jhipsterSampleApplicationApp.location.delete.question');
