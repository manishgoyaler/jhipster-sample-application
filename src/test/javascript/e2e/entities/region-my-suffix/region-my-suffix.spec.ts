...
import { RegionComponentsPage, RegionDeleteDialog, RegionUpdatePage } from './region-my-suffix.page-object';
...
describe('Region e2e test', () => {
  ...
  it('should load Regions', async () => {
    await navBarPage.goToEntity('region-my-suffix');
    regionComponentsPage = new RegionComponentsPage();
    await browser.wait(ec.visibilityOf(regionComponentsPage.title), 5000);
    expect(await regionComponentsPage.getTitle()).to.eq('jhipsterSampleApplicationApp.region.home.title');
  ...
  it('should load create Region page', async () => {
    ...
    expect(await regionUpdatePage.getPageTitle()).to.eq('jhipsterSampleApplicationApp.region.home.createOrEditLabel');
  ...
  it('should delete last Region', async () => {
    ...
    expect(await regionDeleteDialog.getDialogTitle()).to.eq('jhipsterSampleApplicationApp.region.delete.question');
